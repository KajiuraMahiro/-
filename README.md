data_activity_report は、日次のデバイス活動ログ（CSV/TSV）から、
	•	デバイス×日付の集計（件数・合計値）
	•	デバイス単位の稼働サマリ（稼働日数、営業推定日数、稼働率、最大連続停止日数、初回/最終観測日）
...
ロジック概要
	1	CSV/TSVを自動判定で読み込み、型を正規化
	2	期間指定（lookback_days）で抽出（任意）
	3	ストア×日付で「営業推定日」土台を作成（その日に1行でもあれば営業）
	4	デバイス×日付で日次集計（event_count, total_value の合計）
	5	営業日土台に重ねて、event_count>0 を「稼働」と判定
	6	デバイス単位に稼働日数・営業推定日数・稼働率・最大連続停止日数を算出
	7	Excel/CSV/ログ出力# -

# -*- coding: utf-8 -*-
"""
data_activity_report.py (portfolio-safe)
- Input: input/transaction_log.csv  (TSV/CSV both OK; header row required)
- Output: output/activity_report.xlsx   (Sheets: "DeviceDaily", "DeviceSummary")
          output/device_daily.csv       (English columns)
          output/device_summary.csv     (English columns)
Log:      output/logs/data_activity_report.log

Notes
- No external, company-specific names. Columns and paths are generic.
- Adds an "always-on" flag (activity_rate >= threshold) in the summary.
"""
...
def read_tx_csv(path):
    # detect separator by the first line
...
def build_store_day_status(df_tx):
    ...
    # At this point, rows>0 always holds due to groupby, but we keep an explicit flag
...
def main():
    # resolve base dir (supports PyInstaller)
